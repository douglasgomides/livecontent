...
import { ArrowLeft, Target, CheckCircle2, ShieldAlert, MessageCircleQuestion, Heart, Lightbulb, TrendingUp, Wallet, Gauge } from 'lucide-react';
...
import type { PatientSignal, Product, LeadCapture } from '@/types/session';
...
// com selo "Exemplo" visível, nunca com achado real.
...
// Inteligência COMERCIAL — o que o paciente diz (objeção, sentimento, dúvida,
// sinal de compra), como isso se cruza com o funil/formato, e quanto disso já
// virou receita real. Deliberadamente separada da Inteligência de Conteúdo
// (funil de temas, aprovação/CFM, palavras-chave, em /app/insights): uma é
// sobre o que você publica, a outra é sobre o que fecha venda.
export default function Commercial() {
  ...
  const confirmedRevenue = useMemo(() => originRoiRows.reduce((a, r) => a + r.revenue, 0), [originRoiRows]);
    ...
    <div className="space-y-8 pb-24 md:pb-0">
      <div>
        ...
        <p className="text-primary text-xs tracking-[0.3em] uppercase mb-2 flex items-center gap-2">
          <Target className="h-3.5 w-3.5" /> Inteligência comercial
        </p>
        <h1 className="font-serif text-4xl mb-2">Objeções, sentimento e argumentos pra vender melhor</h1>
        <p className="text-muted-foreground">
          Extraído automaticamente da fala do paciente durante suas consultas — objeção, sentimento,
          dúvida e sinal de compra, cruzados com o que já virou receita de verdade.
        </p>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <KpiCard
              ...
              value={objectionData[0]?.category ?? '—'}
              sublabel={objectionData[0] ? `${objectionData[0].quantidade}x identificada` : 'sem objeção ainda'}
              tone="warning"
            />
            <KpiCard
              icon={CheckCircle2}
              label="Sua taxa de fechamento"
              value={myClosingRate ? `${Math.round(myClosingRate.rate * 100)}%` : '—'}
              sublabel={myClosingRate ? `${myClosingRate.sample} consulta(s) resolvida(s)` : `mínimo ${MIN_MY_OFFERS_FOR_BENCHMARK} pra calcular`}
              tone="primary"
            />
            <KpiCard
              icon={Wallet}
              label="Receita confirmada"
              value={confirmedRevenue > 0 ? confirmedRevenue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : '—'}
              sublabel="upsell aceito, com origem rastreada"
              tone="success"
            />
          </div>
