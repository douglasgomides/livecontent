...
import { ArrowLeft, ClipboardList, Copy, Check, Link2 } from 'lucide-react';
import { toast } from 'sonner';
import { Button } from '@/components/ui/button';
import {
  Select, SelectContent, SelectItem, SelectTrigger, SelectValue,
} from '@/components/ui/select';
import { fetchPreConsultationResponses, linkPreConsultationResponse, fetchRecentSessionsForLinking } from '@/lib/db';
import { getUserId } from '@/lib/store';
import { PRE_CONSULT_QUESTIONS } from '@/lib/preConsultQuestions';
import type { PreConsultationResponse } from '@/types/session';
...
export default function PreConsultation() {
  ...
  const refresh = async () => {
    ...
    setLoading(true);
    try {
      const [respList, sessList] = await Promise.all([
    ...
    <div className="space-y-8 pb-24 md:pb-0">
      ...
      <section className="border border-border/60 rounded-xl p-5">
        ...
        <p className="text-xs text-muted-foreground mt-3">
          Perguntas fixas por enquanto: {PRE_CONSULT_QUESTIONS.map(q => q.label).join(' · ')}
        </p>
      </section>
