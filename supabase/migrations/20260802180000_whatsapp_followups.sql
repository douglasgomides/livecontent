...
-- Uma mensagem de acompanhamento por (consulta, resposta de pré-consulta) —
-- rascunho gerado a partir da objeção/sentimento/produto já extraídos, sempre
-- revisável, só sai depois de aprovação explícita do médico.
create table if not exists public.whatsapp_followups (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  session_id uuid not null references public.sessions(id) on delete cascade,
  preconsult_response_id uuid not null references public.preconsultation_responses(id) on delete cascade,
  phone text not null,
